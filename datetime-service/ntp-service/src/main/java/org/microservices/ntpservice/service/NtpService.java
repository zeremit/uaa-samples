...
public class NtpService {
    ...
    @Value("${ntp.server.timeout:0}")
    private Integer ntpServerTimeout;

    @Value("${timezone}")
    private String timezone;

    @RequestMapping(value = "/full_date", method = RequestMethod.GET)
    public long getFullDateInMillis() throws IOException, InterruptedException {
        Thread.sleep(ntpServerTimeout);
        NTPUDPClient ntpudpClient = new NTPUDPClient();
        TimeInfo time = ntpudpClient.getTime(InetAddress.getByName(ntpServerUrl));
